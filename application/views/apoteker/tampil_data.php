    	...
    	<div class="modal-content">
      		...
      		<div class="modal-body">
      			<div class="row">
                    <div class="col-lg-12">
                    	<div id="message"></div>
                        <?php echo form_open('apoteker','id="form" class="form-horizontal col-md-12"') ?>              	 
                          	
                              	<div class="form-group">
                                	<label class="col-sm-4 control-label">Kode Apoteker</label>
	                                <div class="col-sm-8">
	                                    <input type="text" id="kode" class="form-control" name="kdApoteker" value="<?php echo $kode ?>">
...
<script type="text/javascript">
	...
	table = $('#dt-table').DataTable({		            
		...
		"ajax":{
			"url":"<?php echo site_url('apoteker/tampil_data') ?>",
			"type":"POST"
		}
	});
...
$(document).ready(function() {
	...
	{	    
	    ...
	    $('.modal-title').text('Tambah Data Apoteker'); // Set Title to Bootstrap modal title
	}
	...
	function simpan_data(){
	    ...
	    if(save_method == 'tambah'){
	        url = "<?php echo site_url('apoteker/ajax_input')?>";
	    }else{
	        url = "<?php echo site_url('apoteker/ajax_update')?>";
	    }
	...
	function edit_data(kodeApoteker){
	    save_method = 'update';
	    $('#form')[0].reset(); // reset form on modals
	    $('#kode').prop('readonly', true);	  
	    $('.form-group').removeClass('has-error'); // clear error class
	    ...
	    $.ajax({
	        url : "<?php echo site_url('apoteker/ajax_edit/')?>" + kodeApoteker,
	        type: "GET",
	        dataType: "JSON",
	        success: function(data)
	        {
	 
	            $('[name="kdApoteker"]').val(data.kodeApoteker);
	            $('[name="nama"]').val(data.nama);
	            $('[name="tmpLahir"]').val(data.tempatLahir);
	            $('[name="tglLahir"]').val(data.tanggalLahir);
	            ...
	            $('.modal-title').text('Edit Data Apoteker'); // Set Title to Bootstrap modal title
	...
	function hapus_data(kodeApoteker)
	{
	    if(confirm('Anda yakin akan menghapus data dengan kode ' + kodeApoteker + ' ?'))
	    {
	        // ajax delete data to database
	        $.ajax({
	            url : "<?php echo site_url('apoteker/ajax_delete')?>/" + kodeApoteker,
	            type: "POST",
	            dataType: "JSON",
	            success: function(data)
	            {
